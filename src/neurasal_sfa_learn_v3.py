...
if __name__ == "__main__":
    ...
    if nn_args.pre_train_nn:
        ...
        pretrain_nn(train_data, test_data, nn_args.pre_training_size, model, optimizer, nn_args.pre_train_num_epochs)
    ...
    for epoch in range(1, nn_args.num_epochs + 1):
        ...
        for batch in train_loader:
            ...
            sequence_labels = torch.stack([
                seq.get_sequence_label() for seq in batch.sequences
            ]).to(device)  # (BS,)

            seq_loss = sequence_criterion(acceptance_probabilities, sequence_labels.float())
            latent_loss = get_latent_loss(batch, nn_outputs, nn_criterion)
            ...
            sc.update_stats(batch, latent_predictions, sequence_predictions, seq_loss.item(), latent_loss.item())

        eval_model(sc, test_loader, model, sfa_dnnf, cnn_output_size, len(train_loader), epoch)

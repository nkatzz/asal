...
def u_from_probs_any(bank: PredicateBase, prob_seq: torch.Tensor) -> torch.Tensor:
    ...
    ----
    bank : PredicateBase
        Predicate mapper (reused across dimensions).
    prob_seq : float tensor
        Digit probabilities, either (B,T,10) or (B,T,D,10).

    Returns
    -------
    predicate_truths_seq : (B,T,22) or (B,T,22·D) float32
        Per-time predicate soft-truths for the sequence(s).
    """
    if prob_seq.dim() == 3:
        # Univariate: (B,T,10)
        B, T, _ = prob_seq.size()
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------
    dnfs_by_edge = model.transition.extract_all(predicate_names_multi(dim_names, D),
                                                ts=args.extract_th, ta=args.extract_ta)

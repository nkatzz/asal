...
from bbbbbbbb_utils import symbol_names_multi, SymbolDNFEdge, probs_to_symbol_feats, pretty_symbol_clause_grouped
...
class DNFEdge(nn.Module):
    ...
    def forward(self, predicate_truths_t: torch.Tensor, literal_temperature: float = 0.8) -> torch.Tensor:
        ...
        s_act = w_pos + w_neg  # active mass (exclude IGN), (1,R,m)
...
class TransitionLayer(nn.Module):
    ...
    """
        Predicate truths OR symbol feats → per-time row-stochastic transition kernel.
        """
    ...
    def __init__(
            ...
            m: int,
            R: int = 2,
            allowed_edges: List[Tuple[int, int]] | None = None,
            edge_class: type = DNFEdge,
            edge_kwargs: dict | None = None,
    ):
        """
        Transition layer: builds one guard module per (src,dst) edge and a row-logit mask.

        Args
        ----
        n : number of states
        m : input feature size to edge_class (predicate-DNF only; ignored by SymbolDNFEdge)
        R : max number of DNF clauses per edge
        allowed_edges : list of (src,dst) edges that are allowed; others get -inf mask
                        If None, defaults to self-loops and forward edges: {(q,q), (q,q+1)} where valid.
        edge_class : class for the edge guard (DNFEdge, SymbolDNFEdge, pix-style, ...)
        edge_kwargs : extra kwargs passed to edge_class (e.g., num_dims, num_symbols for SymbolDNFEdge)
        """
        super().__init__()
        import inspect
        ...
        # --- Instantiate one guard module per (src,dst) depending on ctor signature ---
        sig = inspect.signature(edge_class)
        params = sig.parameters
        if "m" in params:
            # predicate-DNF style: edge_class(m, R=..., **kwargs)
            self.edges = nn.ModuleList([edge_class(m, R=R, **edge_kwargs) for _ in range(n * n)])
        else:
            # symbol-DNF (or other) style: edge_class(R=..., **kwargs)
            self.edges = nn.ModuleList([edge_class(R=R, **edge_kwargs) for _ in range(n * n)])
        # ------------------------------------------------------------------------------

        # --- Build row-logit mask: 0.0 for allowed edges; -inf for disallowed edges ---
        # Default topology (if none provided): self-loops and forward edges.
        if allowed_edges is None:
            allowed_edges = [(q, q) for q in range(n)]
            allowed_edges += [(q, q + 1) for q in range(n - 1)]

        mask = torch.full((n, n), float("-inf"))  # added to row logits before softmax
        for (src, dst) in allowed_edges:
            if 0 <= src < n and 0 <= dst < n:
                mask[src, dst] = 0.0

        # Keep both a numeric logit-mask and a boolean convenience mask, if you need it.
        self.register_buffer("logit_mask", mask)  # (n,n) 0 or -inf
        self.register_buffer("allowed_bool", mask.isfinite())  # (n,n) True for allowed
        # ------------------------------------------------------------------------------
    ...
    def forward(self, predicate_truths_t: torch.Tensor,
        ...
        # logits = torch.log(guard_strengths.clamp_min(1e-6)) + self.logit_mask
        eps = 1e-6
        logits = torch.logit(guard_strengths.clamp(eps, 1.0 - eps)) + self.logit_mask  # (B,n,n)
    ...
    def extract_all(self, names: List[str], ts: float = 0.75, ta: float = 0.2
        ...
        for src_state in range(self.n):
            for dst_state in range(self.n):
                if not torch.isfinite(self.logit_mask[src_state, dst_state]):
                    continue
...
def compute_f1_from_loader(model: NeSySFA, bank: PredicateBase, loader: DataLoader,
    ...
    for batch in loader:
        ...
        if args.guard == "sym_dnf":
            feature_seq = probs_to_symbol_feats(xb)  # (B,T,D*K)
        else:
            feature_seq = u_from_probs_any(base_predicates, xb)

        y_pred = model(feature_seq, row_temperature=tau_row, literal_temperature=tau_gs)
...
def sweep_f1(model: NeSySFA, bank: PredicateBase, loader: DataLoader,
    ...
    for batch in loader:
        ...
        # u_seq = u_from_probs_any(bank, xb)
        if args.guard == "sym_dnf":
            feat_seq = probs_to_symbol_feats(xb)  # (B,T,D*K)
        else:
            feat_seq = u_from_probs_any(base_predicates, xb)
        y_pred = model(feat_seq, row_temperature=tau_row, literal_temperature=tau_gs)
...
def summarize_edge_usage(model: NeSySFA, bank: PredicateBase, loader: DataLoader,
    ...
    for batch in loader:
        ...
        if args.guard == "sym_dnf":
            ...
            feat_seq = u_from_probs_any(base_predicates, xb)
        y_pred, state_beliefs, transition_matrices, _ = model.forward_with_traces_and_g(feat_seq, tau_row, tau_gs)
...
def print_initial_row_stats(model, bank, loader, row_temp=0.8, lit_temp=1.0, n_batches=1):
    ...
    for _ in range(n_batches):
        ...
        if args.guard == "sym_dnf":
            u = probs_to_symbol_feats(xb)  # (B,T,D*K)
        else:
            u = u_from_probs_any(base_predicates, xb)
...
def print_selector_stats(model):
    ...
    if pos_means:
        print(
            f"[selector] mean IGN={np.mean(ign_means):.3f}  POS={np.mean(pos_means):.3f}  NEG={np.mean(neg_means):.3f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--guard",
                        choices=["dnf", "pix", "pix_hybrid", "sym_dnf"],
                        default="dnf",
                        help="'dnf' (predicate DNF); 'sym_dnf' (symbol-space DNF); "
                             "'pix'/'pix_hybrid' use pix2rule-style units.")
    ...
    parser.add_argument("--extract-ta", type=float, default=0.20,
                        help="Clause α threshold ta for extraction (DNF/hybrid).")

    # Row-temperature annealing (optional)
    parser.add_argument("--tau-row-start", type=float, default=1.0,
                        help="Initial row-softmax temperature (warmer = softer).")
    parser.add_argument("--tau-row-final", type=float, default=0.1,
                        help="Final row-softmax temperature for the last epoch.")
    parser.add_argument("--tau-sched", choices=["linear", "cosine"], default="linear",
                        help="Annealing schedule for tau_row over epochs.")

    # Any-time acceptance auxiliary loss (optional)
    parser.add_argument("--lambda-any", type=float, default=5e-3,
                        help="Auxiliary weight for any-time acceptance BCE (0 disables).")
    ...
    D = xb_sample.shape[2]
    K = xb_sample.shape[3]  # usually 10 for MNIST digits
    base_predicates = PredicateBase()  # still used for predicate-DNF; harmless otherwise

    if args.guard == "sym_dnf":
        # Symbol mode: names per (dimension,symbol) and m = D*K
        names = symbol_names_multi(dim_names, D, K=K)
        ACTIVE_PRED_IDX = None  # mask applies only to predicate mode
        m_predicates = D * K
        print(f"[INFO] sym-DNF mode: D={D}, K={K}, m={m_predicates}")
    else:
        # Predicate mode (your existing path)
        names = predicate_names_multi(dim_names, D)  # length 22*D
        # ... keep/drop handling exactly as you already have ...
        # (this block remains unchanged)
        # after the keep/drop logic, set:
        m_predicates = len(names)

    # ---------------------------------------------------------------------------------------------------------
    # ---------------------------------------------------------------------------------------------------------
    if args.guard == "sym_dnf":
        # Symbol-DNF mode: names per (dimension,symbol), no predicate gating.
        names = symbol_names_multi(dim_names, D, K=K)  # e.g., ['d1_is_0',..,'dD_is_9']
        ACTIVE_PRED_IDX = None  # do not gate symbol features
        m_predicates = D * K
        print(f"[INFO] sym-DNF active features = {m_predicates}: {', '.join(names)}")
    else:
        # Predicate-DNF mode: names are 22 predicates per dimension; allow keep/drop masks.
        names = predicate_names_multi(dim_names, D)  # length = 22*D
        ...
        if keep_str:
            ...
            ACTIVE_PRED_IDX = torch.tensor(idx_keep, dtype=torch.long)
    ...
    elif args.guard in ("pix", "pix_hybrid"):
    ...
    elif args.guard == "sym_dnf":
        # Transition uses SymbolDNFEdge internally
        model = NeSySFA(
            n=args.num_states, m=m_predicates, R=args.num_ors,
            allowed_edges=allowed, accept_state=args.num_states - 1
        ).to(device)
        # swap the transition's edge type in-place
        model.transition = TransitionLayer(
            n=args.num_states, m=m_predicates, R=args.num_ors,
            allowed_edges=allowed,
            edge_class=SymbolDNFEdge,
            edge_kwargs={"num_dims": D, "num_symbols": K}
        ).to(device)
    ...
    def eval_loss() -> float:
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                if args.guard == "sym_dnf":
                    predicate_truths_seq = probs_to_symbol_feats(xb)  # (B,T,D*K)
                else:
                    predicate_truths_seq = u_from_probs_any(base_predicates, xb)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            if args.guard == "sym_dnf":
                ...
                feat_seq = u_from_probs_any(base_predicates, xb)  # (B,T,22*D or masked)

            y_pred, state_beliefs, transition_matrices, guard_strengths = \
                model.forward_with_traces_and_g(
                    feat_seq,
            ...
            for t, g_raw in enumerate(guard_strengths):
                for q in range(model.n - 1):
                    if torch.isfinite(model.transition.logit_mask[q, q]) and torch.isfinite(model.transition.logit_mask[q, q + 1]):
                        g_self = g_raw[:, q, q]
                        g_fwd = g_raw[:, q, q + 1]
    ...
    for (q, qp), clauses in dnfs_by_edge.items():
        if not clauses:
            continue
        if args.guard == "sym_dnf":
            # clauses: list of grouped symbol clauses
            pretty = "  OR  ".join(pretty_symbol_clause_grouped(c) for c in clauses)
        else:
            # predicate mode (your existing pretty_clause)
            pretty = "  OR  ".join(pretty_clause(c) for c in clauses)
        print(f"{q}->{qp}:  {pretty}")
